var dir = [];
function checkCollision(doubleLayerArray, weaponPosition, options){
...
function heroCollider(hero){
    if(hero.dy<12){hero.dy=12}
    if(hero.dx<12){hero.dx=12}
    if(hero.dy>canvas.height-12){hero.dy=canvas.height-12}
    if(hero.dx>canvas.width-12){hero.dx=canvas.width-12}
}
...
function heroCollider2(doubleLayerArray, hero, options){
    ...
    for(var i=0;i<_array.length;i++){
        if(hero.dy>=_array[i][TOP]-con && hero.dy<=_array[i][BOTTOM]+con){yCollision[i]=true}
        if(hero.dx>=_array[i][LEFT]-con && hero.dx<=_array[i][RIGHT]+con){xCollision[i]=true}
        if(yCollision[i]===true && xCollision[i]===true)
        {
            if(dir[i] === "left"){hero.dx=_array[i][LEFT]-con}
            else if(dir[i] === "right"){hero.dx=_array[i][RIGHT]+con}
            else if(dir[i] === "top"){hero.dy=_array[i][TOP]-con}
            else if(dir[i] === "bottom"){hero.dy=_array[i][BOTTOM]+con}
            break;
        }
        else{
            if(hero.dy<=_array[i][TOP]-con && xCollision[i]===true){dir[i]="top"}
            else if(hero.dy>=_array[i][BOTTOM]-con && xCollision[i]===true){dir[i]="bottom"}
            else if(hero.dx<=_array[i][LEFT]-con && yCollision[i]===true){dir[i]="left"}
            else if(hero.dx>=_array[i][RIGHT]-con && yCollision[i]===true){dir[i]="right"}
        }
    }
}
